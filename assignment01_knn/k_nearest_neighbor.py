...
class KNearestNeighbor:
    ...
    def compute_distances_no_loops(self, X):
        ...
        dists = np.zeros((num_test, num_train))
        X_sq = np.tile(np.sum(X * X, axis=1).reshape((num_test, 1)), (1, num_train))
        X_A = 2 * (X.dot(self.X_train.T))
        dists = np.sqrt(X_sq - X_A + np.tile(np.sum(self.X_train * self.X_train, axis=1), (num_test, 1)))
    ...
    def predict_labels(self, dists, k=1):
        ...
        for i in range(num_test):
            ...
            closest_y = np.take_along_axis(self.y_train, np.argsort(dists[i])[:k], axis=0)
